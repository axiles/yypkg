...
val to_buffer_gen :
  buf:'a ->
  add_char:('a -> char -> unit) ->
  add_string:('a -> string -> 'b) -> t -> unit
...
module Tmp_file :
  sig
    val prng : Random.State.t option ref
    val temp_file_name : string -> string -> string
    val open_temp_file :
      ?perm:int -> string -> string -> string * out_channel
  end
val save_of_output :
  ?perm:int -> (out_channel -> 'a -> 'b) -> string -> 'a -> unit
...
val mk_cont_state :
  ...
  'a -> cont_state:Cont_state.t -> (bigstring, 'b) parse_result
val mk_cont :
  string ->
  (Annot.stack parse_state ->
