...
let expand pkg i p =
  ...
  in
     ...
     * and doesn't recognize the option *)
     ...
     let xx =
       if BSD = Lib.tar_kind
       then List.rev_map filter_bsdtar_output x
       else List.rev x
     in List.rev_map (Lib.strip_component ~prefix: p (l - 1)) xx)
...
let open_package package =
  ...
  let s = String.concat "\n" l in script_of_sexp (Sexp.of_string s)
