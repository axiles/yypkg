  ...
  in
    ...
    then Args.print_spec 0 (Args.usage_msg cmd_line_spec)
      ...
      (let cmd_line = Args.parse cmd_line_spec Sys.argv in
       ...
       in
         if ("-init", []) = (action, actionopts)
         then
           (* setups a few things for correct operation of yypkg, see yypkg/init.ml*)
           Init.init prefix
         else (* Some operations are relative to the prefix so chdir to it *)
            ...
            in
              (Sys.chdir prefix;
               ...
               | (* install, accepts one package at a time *)
                   ("-install", [ Args.Val s ]) ->
                   let s = FilePath.DefaultPath.make_absolute old_cwd s
                   in Db.update (Install.install s (Conf.read ()))
