...
let cmd_line_spec =
  [ ("-prefix", []); ("-install", []); ("-uninstall", []); ("-list", []);
    ("-config", [ ("-setpreds", []); ("-delpreds", []); ("-listpreds", []) ]);
    (* not handled currently: "-regen", []; *) ("-init", []) ]
...
let () = (* FIXME: fix the usage message... *)
  let usage_msg = "bad usage, will make better error message tomorrow"
  in
