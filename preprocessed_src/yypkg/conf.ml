...
let set conf (binding, value) =
  let conf = List.remove_assoc binding conf in (binding, value) :: conf
  
let unset conf binding = List.remove_assoc binding conf
  
let read () = conf_of_sexp (Disk.read conf_path)
...
let write conf =
  ...
  let sorted_conf = List.stable_sort compare conf
  in Disk.write conf_path (sexp_of_conf sorted_conf)
  
(* read the conf, run the function, write the database to disk
 * if fail raises an exception, nothing will be written :-) *)
let update f = write (f (read ()))
