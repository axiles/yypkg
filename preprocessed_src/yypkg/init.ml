...
let init prefix = (* On windows, we need an absolute filename it seems *)
   ...
   * default to *)
     if "Win32" = Sys.os_type
     then FileUtil.cp binaries (make_absolute prefix "sbin")
     else ();
