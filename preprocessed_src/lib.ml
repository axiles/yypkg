...
let read pid descr =
    ...
    let rec read_once_rc accu =
      ...
      | ([ _ ], _, _) ->
          (match Unix.read descr s 0 160 with
           ...
           | (* got less than asked, return *) l ->
               (String.sub s 0 l) :: accu)
      | (* ok, we got a timeout: return *) _ -> accu
    in read_once_rc [] in
  let rec read_rc pid descr accu =
    ...
    | (* still alive: read and start again *) (0, _) ->
        read_rc pid descr ((read_once descr) :: accu)
...
let binary_path =
  ...
  in
    if FilePath.DefaultPath.is_relative dirname
    then filename_concat [ Sys.getcwd (); dirname ]
    else dirname
...
let (tar, tar_kind, xz, gzip, bzip2, named_pipe) =
  ...
  | (* we don't set named_pipe for unix and cygwin because it's not used *)
      "Unix" | "Cygwin" -> ("tar", GNU, "xz", "gzip", "bzip2", "")
  | "Win32" ->
...
let decompress_untar tar_args input =
   ...
   * up...) so we'll try to take advantage of that *)
    ...
    | ("Win32", false) ->
        ...
        let pid_t =
          if not (List.mem "-O" (Array.to_list tar_args))
          then Unix.create_process t.(0) t Unix.stdin Unix.stdout t_in
          else Unix.create_process t.(0) t Unix.stdin t_in Unix.stderr
        in read pid_t t_out
    | _ ->
        ...
        let (t_out, t_in) = Unix.pipe () in
        let pid_c =
          Unix.create_process c.(0) c Unix.stdin c_in Unix.stderr in
