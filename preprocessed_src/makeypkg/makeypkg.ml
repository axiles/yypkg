...
type cmd_line =
  { output : string; folder : string; folder_dirname : string;
    ...
    pkger_name : string; pkger_email : string; descr : string; arch : string;
    compressor : string
  }
...
let parse_command_line () =
  ...
  let lst =
    ...
    [ ("-o", (Arg.Set_string output), "set output folder");
      ...
      ("-compressor", (Arg.Set_string compressor), "xz, gzip or bzip2") ] in
  ...
  in
    ...
    if
      List.exists (( = ) (ref ""))
        [ output; folder; pkg_name; version; packager_name; packager_email;
          description; arch; compressor ]
...
let () =
  ...
  let tar_args =
    ...
    |] in
  let snd =
    match cmd_line.compressor with
    | "xz" ->
        [| xz; "--x86";
    ...
    | "bzip2" -> [| bzip2; "-9" |]
    | _ -> assert false in
  let output = Filename.concat cmd_line.output (output_of_cmdline cmd_line)
  in tar_compress tar_args snd output
